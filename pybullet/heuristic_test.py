...
def heuristic_controller_1(env, visualize=False):
    obs, _ = env.reset()
...
def heuristic_controller_2(env, visualize=False):
    obs, _ = env.reset()
    ball_pos, net_pos, xy_dist = None, None, None
    ...
    for _ in range(env.max_ep_steps + 0):
        ...
        base_target = np.clip(-1 * np.arctan2(ball_y, ball_x), -1.57, 1.57)
        hyp = np.sqrt(np.square(ball_x) + np.square(ball_y)) - 1.0
        shoulder_target = np.clip((hyp / 2.5) * 1.57, 0, 1.57)  # Perform linear interpolation
...
def main():
    ...
    for i in range(num_trials):
        caught = heuristic_controller_2(env, visualize=visualize)
    ...
    with open('results/heuristic_2_w_noise.csv', mode='w', newline='') as file:
